    ...
    <v-main>
      <v-container>
        ...
        <p>
          <v-row>
            <v-col sm="6">
              <router-link v-for="crumb in breadcrumbs" :key="crumb.link" :to="`/explorer${crumb.link}`">/{{ crumb.text }}</router-link>
            </v-col>
            <v-col sm="6">
              <v-row >
                <v-col>
        ...
        >
          <template v-slot:item.name="{item}" @click="refresh()">
            <router-link v-if="item.isFolder" :to="`/explorer/${item.containerName}/${item.prefix}`">{{ item.name }}</router-link>
          ...
          <template v-slot:item.actions="{ item }">
            <v-btn v-if="!item.isFolder" icon @click="copyToClipboard(`${config.downloadUri}/${item.containerName}/${item.prefix}`)">
